...
public class CtrlLineShape extends BaseCtrlShape {
	...
	public void draw(Graphics2D g) {
		...
		shape = new Rectangle2D.Double(p.getX(), p.getY(), ShapeHelper.CTRL_SHAPE_SIZE,
				ShapeHelper.CTRL_SHAPE_SIZE);
		g.draw(new Line2D.Double(p.getX(), p.getY(),
				p.getX() + ShapeHelper.CTRL_SHAPE_SIZE, p.getY() + ShapeHelper.CTRL_SHAPE_SIZE));
		g.draw(new Line2D.Double(p.getX(), p.getY() + ShapeHelper.CTRL_SHAPE_SIZE, p.getX()
				+ ShapeHelper.CTRL_SHAPE_SIZE, p.getY()));
		drawLineHoverBox(g);
	}
